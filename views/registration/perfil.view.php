...
<main class="contenedor" style="padding: 2rem 0 2.5rem;">
  ...
  <section class="card shadow-sm border-0 rounded-4 mb-4">
    <div class="card-body p-4">
      ...
      <form action="#" method="get" class="row g-2 align-items-center mb-3">
      ...
      <div class="table-responsive">
        <table class="table table-hover align-middle mb-0">
          <thead>
            <tr><th>Usuario</th><th>Nombre</th><th>Estado</th><th style="width:160px;">Acción</th></tr>
          </thead>
          <tbody>
              ...
              <tr>
                <td class="d-flex align-items-center gap-2">
                  <img src="/<?= htmlspecialchars($u['avatar'] ?? 'imagenes/default-profile.jpg') ?>"
                       alt="Avatar" style="width:36px;height:36px;border-radius:999px;object-fit:cover;">
    ...
    <div class="card-body p-4">
      <div class="d-flex flex-column flex-md-row align-items-md-end gap-3" style="margin-top:-70px;">
        ...
        <div class="flex-grow-1">
          <h2 class="m-0"><?= htmlspecialchars($perfil['nombre'] ?? '') ?></h2>
          <p class="m-0" style="opacity:.8;">@<?= htmlspecialchars($perfil['username'] ?? '') ?></p>
        </div>
      ...
      <div class="row g-3">
        <div class="col-12 col-md-6">
          <div class="p-3 rounded-4" style="background:var(--bg);border:1px solid var(--border);">
            <p class="m-0"><strong><i class="fas fa-envelope me-2"></i>Email:</strong> <?= htmlspecialchars($perfil['email'] ?? '') ?></p>
          </div>
        </div>
        <div class="col-12 col-md-6">
          <div class="p-3 rounded-4" style="background:var(--bg);border:1px solid var(--border);">
            <p class="m-0"><strong><i class="fas fa-location-dot me-2"></i>Ubicación:</strong> <?= htmlspecialchars($perfil['ciudad'] ?? '—') ?></p>
          </div>
        </div>
        <div class="col-12 col-md-6">
          <div class="p-3 rounded-4" style="background:var(--bg);border:1px solid var(--border);">
            <p class="m-0"><strong><i class="fas fa-cake-candles me-2"></i>Nacimiento:</strong> <?= htmlspecialchars($perfil['fechaNacimiento'] ?? '—') ?></p>
          </div>
        </div>
        <div class="col-12 col-md-6">
          <div class="p-3 rounded-4" style="background:var(--bg);border:1px solid var(--border);">
            <p class="m-0"><strong><i class="fas fa-venus-mars me-2"></i>Género:</strong> <?= htmlspecialchars($perfil['genero'] ?? '—') ?></p>
          </div>
        </div>
      </div>
    </div>
        ...
        <article class="card border-0 shadow-sm rounded-4 mb-3">
          <div class="card-body">
            ...
            <?php if (!empty($post['imagen'])): ?>
              <img src="/<?= htmlspecialchars($post['imagen']) ?>" alt="Imagen de post"
                   class="img-fluid rounded-4 border" style="max-height:320px;object-fit:cover;width:100%;">
            <?php endif; ?>
            <div class="d-flex gap-3 mt-3" style="font-size:14px;opacity:.85;">
              <span><i class="fas fa-thumbs-up me-1"></i> <?= $post['likes'] ?? 0 ?></span>
              <span><i class="fas fa-comment me-1"></i> <?= $post['comentarios'] ?? 0 ?></span>
            </div>
          </div>
        </article>
      <?php endforeach; ?>
